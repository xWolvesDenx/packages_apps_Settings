...
public class ROMVersionPreferenceController extends AbstractPreferenceController
    ...
    public boolean isAvailable() {
        return !TextUtils.isEmpty(SystemProperties.get(PROPERTY_VALIDUS_VERSION));
    }

    @Override
    public String getPreferenceKey() {
        return KEY_VALIDUS_VERSION;
    }

    @Override
    public void displayPreference(PreferenceScreen screen) {
        super.displayPreference(screen);
        final Preference pref = screen.findPreference(KEY_VALIDUS_VERSION);
        if (pref == null) return;
        String version = SystemProperties.get(PROPERTY_VALIDUS_VERSION);
